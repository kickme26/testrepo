...
func generateKey(pts []int, ma map[string]int)(bool, string,[]int){
	...
	// fmt.Println("_________________________>",converted_arr,copy_pts)
...
func validateCoordinates(apts,bpts,cpts,dpts Coordinates, rec map[string]int)(checkkey bool,string_pairs string,vertices []int){
    if dpts != apts && cpts != dpts && bpts != dpts && apts != bpts && bpts != cpts && apts != cpts{
        allpts := []int{apts.X,apts.Y,bpts.X,bpts.Y,
        cpts.X,cpts.Y,dpts.X,dpts.Y}
       // fmt.Println("////////",allpts)
        checkkey,string_pairs,vertices := generateKey(allpts[:],rec)
        return checkkey,string_pairs,vertices
    }else{
       //  fmt.Println("_______>",checkkey,string_pairs,vertices)
         return checkkey,string_pairs,vertices }
}
        ...
        for bpts:=0;bpts<len(arr);bpts++{
            for cpts:=0;cpts<len(arr);cpts++{
                for dpts:=0;dpts<len(arr);dpts++{
                   checkkey,string_pairs,vertices:= validateCoordinates(arr[apts],arr[bpts],arr[cpts],arr[dpts],rectpoints)
                     if checkkey{
                         dpts++
                         continue
                     }
